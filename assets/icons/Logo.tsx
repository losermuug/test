...
// ── Custom numeric keypad (no system keyboard needed) ─────
...
const CustomNumpad = ({
  ...
  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
    {KEYS.map((k, i) => {
      if (k === '') return <View key={i} style={{ width: '30%' }} />;
      const isBackspace = k === '⌫';
      return (
        <TouchableOpacity
          key={i}
          ...
          style={{
            ...
            paddingVertical: 16,
            borderRadius: 16,
            backgroundColor: isBackspace ? C.indigoSoft : C.surface,
            borderWidth: 1.5,
            borderColor: isBackspace ? C.indigo : C.border,
            alignItems: 'center',
          }}
        >
          <Text style={{
            ...
            fontWeight: '700',
            color: isBackspace ? C.indigo : C.textPrimary,
          }}>
            {k}
          </Text>
        </TouchableOpacity>
          ...
          <Animated.View entering={FadeInUp.duration(700)} style={{ alignItems: 'center', marginTop: 40 }}>
            <Image 
              ...
              style={{
                width: width * 0.85,
                height: width * 0.85,
                marginBottom: -width * 0.2, // Pushing the text closer
            ...
            <Text style={{ fontSize: 15, color: C.textSecondary, marginTop: 8, textAlign: 'center', lineHeight: 22 }}>
              Гэр бүлийн санхүүгийн{'\n'}ухаалаг туслагч
            </Text>

            {/* Feature badges */}
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 24 }}>
              {[
                { label: 'Аюулгүй', color: C.mint, bg: C.mintSoft },
                { label: 'Хялбар', color: C.indigo, bg: C.indigoSoft },
                { label: 'Тоглоомлог', color: C.peach, bg: C.peachSoft },
              ].map(b => (
                <View key={b.label} style={{
                  backgroundColor: b.bg, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6,
                }}>
      ...
      <View style={{ flex: 1, backgroundColor: C.cream }}>
        ...
        <SafeAreaView style={{ flex: 1 }}>
          ...
          >
            ...
            {/* User cards */}
            <Animated.View entering={FadeInDown.duration(500).delay(150)} style={{ gap: 10, marginBottom: 28 }}>
              ...
              {state.users.map(user => {
                ...
                return (
                  <TouchableOpacity
                    key={user.id}
                    onPress={() => { setSelectedUser(user.id); setError(''); setPin(''); }}
                    activeOpacity={0.75}
                    style={{
                      flexDirection: 'row', alignItems: 'center', padding: 14,
                      borderRadius: 18,
                      backgroundColor: isSelected ? softColor : C.surface,
                      borderWidth: 1.5, borderColor: isSelected ? accentColor : C.border,
                    }}
                  >
                    <View style={{
                      width: 48, height: 48, borderRadius: 16, backgroundColor: accentColor,
            ...
            {/* PIN + custom numpad — no system keyboard */}
            {selectedUser && (
              <Animated.View entering={FadeInDown.duration(400)} style={{ gap: 16 }}>
                  ...
                  {[0, 1, 2, 3].map(i => {
                    ...
                    return (
                      <View key={i} style={{
                        ...
                        borderWidth: 1.5, borderColor: filled ? C.indigo : C.border,
                      }}>
                        {filled && <View style={{ width: 12, height: 12, borderRadius: 6, backgroundColor: '#fff' }} />}
                      </View>
                    );
                  })}
                </View>
                ...
                {/* Custom numpad */}
                <CustomNumpad onPress={handleNumpad} />
              </Animated.View>
            )}
          </ScrollView>
        </SafeAreaView>
